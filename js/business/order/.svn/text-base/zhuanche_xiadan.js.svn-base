...
window.addEventListener('loading', function(event) {
	...
	document.getElementById("ZHUANCHEjianyibaojia").innerHTML = "约" + shiji + "元";
});
	...
	var transit = new BMap.DrivingRoute(map, {
		...
		onSearchComplete: function(results) {
			...
			if(allkm > 5000) {
				...
				shiji = parseInt(qibu + dykm * xishu + parseInt(huoyun));
				console.log("实际1：" + shiji);
			} else {
				yuji = parseInt(qibu);
				shiji = parseInt(qibu + parseInt(huoyun));
				console.log("实际2：" + shiji);
			}
			document.getElementById("ZHUANCHEjianyibaojia").innerHTML = "约" + shiji + "元";
			document.querySelector("#next_page button").setAttribute("class", "mui-btn mui-btn-block mui-btn-primary");
		}
...
document.getElementById("next_page").addEventListener('tap', function(e) {
	...
	for(var i1 = 1; i1 <= sss; i1++) {
		...
		console.log(addressjingting_lng_end)
	...
	var webviewShow = plus.webview.getWebviewById('order-confirm');

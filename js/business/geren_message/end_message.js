...
//获取订单页面传过来的类型，如果是经停点为1，目的地为2
//window.addEventListener('order_type', function(event) {
//	//获得事件参数
...
document.getElementById("save_shouhuo").addEventListener('tap', function() {
	//	document.getElementById("save_shouhuo").setAttribute("class","mui-btn mui-btn-block mui-btn-primary mui-disabled");
	var address_name_end = document.getElementById("ADDRESS_NAME_END").value; // 终点
	var address_lng_end = document.getElementById("ADDRESS_LNG_END").value; // 终点经度
	var address_lat_end = document.getElementById("ADDRESS_LAT_END").value; // 终点纬度
	var address_name_end_bieming = document.getElementById("ADDRESS_NAME_END_BIEMING").value.trim(); // 终点简称
	var person_shouhuo = document.getElementById("PERSON_SHOUHUO").value.trim(); // 发货人
	var person_shouhuo_phone = document.getElementById("PERSON_SHOUHUO_PHONE").value.trim(); // 电话
	var phone_validate = /(\(\d{3,4}\)|\d{3,4}-|\s)?\d{7,14}/;
	if(address_lng_end == "") {
		mui.toast("请从下拉列表中选择输入的收货地址");
		//		document.getElementById("save_shouhuo").setAttribute("class","mui-btn mui-btn-block mui-btn-primary");
		return false;
	}
	...
	if(status == 1) {
		...
		mui.fire(xinzeng_luxian, "data_end_message", {
	...
	if(status == 2) {
		...
		mui.fire(xiugai_luxian, "data_end_message", {
			ADDRESS_NAME_END: address_name_end,
			ADDRESS_LNG_END: address_lng_end,
			ADDRESS_LAT_END: address_lat_end,
			ADDRESS_NAME_END_BIEMING: address_name_end_bieming,
	...
	if(status == 3) {
		...
		mui.fire(main3, "data_son_shouhuo_to_ORDER", {
			/*mui.fire(zhuanche_xiadan, "data_end_message", {*/
			ADDRESS_NAME_END: address_name_end,
			ADDRESS_LNG_END: address_lng_end,
			ADDRESS_LAT_END: address_lat_end,
			ADDRESS_NAME_END_BIEMING: address_name_end_bieming,
